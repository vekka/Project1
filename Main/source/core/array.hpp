...
public:
	...
	Array()
		...
		strategy(ALLOC_STRATEGY_DOUBLE), freeWhenDestroyed(true), isSorted(true)
	{
	}
	...
	*/
	void Reallocate( uint32 new_size, bool canShrink = true )
	{
		if (allocated == newSize)
			return;
		if (!canShrink && (newSize < allocated))
			return;

		T* oldData = data;

		data = allocator.allocate(newSize); //new T[new_size];
		allocated = newSize;

		// copy old data
		int32 end = used < newSize ? used : newSize;
	...
	application. */
	void SetPointer( T* newPointer, uint32 size, bool isSorted = false, bool freeWhenDestroyed = true )
	{
		clear();
		data = newPointer;
		allocated = size;
		...
		this->freeWhenDestroyed = freeWhenDestroyed;
	}
	...
	int32 BinarySearch( const T &element )
	{
		Sort();
		return BinarySearch(element, 0, used - 1);
	}
	...
	\return Position of the searched element if it was found,
	otherwise -1 is returned. */
	int32 BinarySearch( const T& element ) const
	{
		if (isSorted)
			return BinarySearch(element, 0, used - 1);
		else
	...
	is returned. */
	int32 BinarySearch( const T &element, int32 left, int32 right ) const
	{
		if (!used)
			return -1;
		int32 m;
	...
	otherwise -1 is returned. */
	int32 BinarySearchMulti( const T &element, int32 &last )
	{
		Sort();
		int32 index = BinarySearch(element, 0, used - 1);
		if (index < 0)
			return index;
		...
		// look linear up
		while (last < (int32)used - 1 && !(element < data[last + 1]) && !(data[last + 1] < element))
		{
			last += 1;
		}
	...
	\return Position of the searched element if it was found, otherwise -1
	is returned. */
	int32 LinearSearch( const T& element ) const
	{
		for (uint32 i = 0; i<used; ++i)
			if (element == data[i])
				return (s32)i;
	...
	\return Position of the searched element if it was found, otherwise -1
	is returned. */
	int32 LinearReverseSearch( const T &element ) const
	{
	...
	void Erase(uint32 index, int32 count)
	{
